...
export default function PersonalAnalyticsPage() {
  ...
  if (!experience || !user) return <div className="flex h-screen items-center justify-center"><Spinner /></div>;
  ...
  // Fetch parlays for analytics
  const { data: parlaysData, isLoading: isLoadingParlays } = useQuery({
    queryKey: ["my-parlays-analytics", experienceId],
    queryFn: async () => {
      const response = await fetch(`/api/parlays?experienceId=${experienceId}&limit=500`);
  ...
  const analytics = useMemo(() => {
    ...
    filteredBets.forEach(item => {
      ...
      if (dollarsInvested && item.result !== "pending") {
        const date = new Date(item.createdAt).toISOString().split("T")[0];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        {Object.keys((analytics as any).leagueBreakdown ?? {}).length > 0 && (
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <div className="space-y-4">
                {Object.entries((analytics as any).leagueBreakdown)
                  .sort(([, a]: any, [, b]: any) => b.total - a.total)
                  .map(([leagueKey, stats]: any) => {
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>How you perform at different odds levels</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <Table>
